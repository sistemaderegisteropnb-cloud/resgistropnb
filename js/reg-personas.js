window.initRegPersonas = function() {
    ...
    const fechaNac = document.getElementById('p_fecha_nac'), edadInput = document.getElementById('p_edad');
    if (fechaNac && edadInput) fechaNac.addEventListener('change', () => {
    ...
    const cedulaInput = document.getElementById('p_cedula'), tlfNumInput = document.getElementById('p_tlf_num');
    ...
    // 🔹 3. VALIDACIÓN CÉDULA
    const cedulaStatus = document.getElementById('cedula-status'); let cedulaCheckTimeout = null;
    async function verificarCedula(cedula) {
        if (!cedulaStatus) return false;
        cedulaStatus.className = 'cedula-status checking'; cedulaStatus.textContent = '🔍 Verificando...';
        ...
        try {
            ...
            if (data) { cedulaStatus.className = 'cedula-status error'; cedulaStatus.textContent = '⚠️ Esta cédula ya está registrada'; cedulaInput?.classList.add('cedula-duplicate'); return true; }
        ...
        } catch (e) { console.warn('⚠️ Verificación fallida:', e.message); cedulaStatus.className = 'cedula-status'; cedulaStatus.textContent = ''; return false; }
    }
    if (cedulaInput) {
        cedulaInput.addEventListener('input', function() { const val = this.value.trim(); if (val.length < 7) { if(cedulaStatus){cedulaStatus.className='cedula-status';cedulaStatus.textContent='';} this.classList.remove('cedula-duplicate'); return; } if (cedulaCheckTimeout) clearTimeout(cedulaCheckTimeout); cedulaCheckTimeout = setTimeout(() => verificarCedula(val), 500); });
        cedulaInput.addEventListener('blur', function() { if(this.value.trim().length >= 7) verificarCedula(this.value.trim()); });
    }

    // 🔹 4. ENVÍO
    const form = document.getElementById('form-reg-personas'), btn = form?.querySelector('.btn-submit'), msg = document.getElementById('msg-reg-personas');
    const mostrarError = (t) => { if(msg){msg.textContent='❌ '+t; msg.className='msg error'; msg.style.display='block';} };
    if (!form || !btn) { console.error('❌ Elementos no encontrados'); return; }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const estCm = window.convertirEstatura(); if (!estCm || estCm < 50 || estCm > 230) { mostrarError('Estatura inválida (0.50m - 2.30m).'); document.getElementById('p_estatura')?.focus(); return; }
        ...
        try {
            ...
            const files = { front: document.getElementById('foto_frontal').files[0], izq: document.getElementById('foto_perfil_izq').files[0], der: document.getElementById('foto_perfil_der').files[0] };
            ...
            const urls = { front: await upload(files.front, paths.f), izq: await upload(files.izq, paths.i), der: await upload(files.der, paths.d) };
            ...
            form.reset(); if(edadInput) edadInput.value=''; document.querySelectorAll('.hidden-field').forEach(e=>e.style.display='none'); document.querySelectorAll('.img-preview').forEach(e=>e.style.display='none'); if(cedulaStatus){cedulaStatus.className='cedula-status';cedulaStatus.textContent='';}
        } catch (err) {
            console.error('Error:', err);
            let m = 'Error inesperado. Intente nuevamente.';
            if (err.message.includes('cedula') || err.message.includes('23505')) m = 'Esta cédula ya está registrada.';
            else if (err.message.includes('tlf') || err.message.includes('check')) m = 'Formato de teléfono inválido o incompleto.';
